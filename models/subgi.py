...
class GNNDiscLayer(nn.Module):
    ...
    def forward(self, g, v, edges, depth=1):
        if depth == 1:
            ...
            self.layer_1 = False
        g.apply_edges(self.edge_concat, edges)
...
class SubGDiscriminator(nn.Module):
    ...
    def forward(self, nf, emb, features):
        # nf - node flow (sampling result)
        # emb - node embedding z_i
        # features - node feature

        # extract original edges from node flow
        # reverse edges for g_i tilde
        reverse_edges = []
        for i in range(nf.num_blocks):
            u,v = self.g.find_edges(nf.block_parent_eid(i))
        ...
        # extract subgraph
        # save node_embedding --> root and node_feature --> x
        ...
        small_g.ndata['m']= torch.zeros_like(emb[small_g.ndata['_ID']]) # zero embedding?
...
class SubGI(nn.Module):
    def __init__(self, g, in_feats, n_hidden, n_layers, activation, dropout, model_id=2, pretrain=None):
        # g: ???
    ...
    def forward(self, features, nf):
        ...
        perm = torch.randperm(self.g.number_of_nodes()) # get random shuffle indices
        negative = positive[perm] # why extract from positive?        
